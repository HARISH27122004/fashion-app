...
/* ─────────────────────────────────────────────
   Zone centres & sizes (% of SVG container)
───────────────────────────────────────────── */
  ...
  return (
    ...
    >
      ...
      <g filter="url(#tshirt-shadow)">
        <path d="M20 90 L0 190 L65 210 L80 130 Z"                                                                                fill={fill} stroke={shadow} strokeWidth="1.5" />
        <path d="M320 90 L340 190 L275 210 L260 130 Z"                                                                           fill={fill} stroke={shadow} strokeWidth="1.5" />
        <path d="M80 80 C80 80 100 65 140 60 L150 80 C155 90 165 96 170 96 C175 96 185 90 190 80 L200 60 C240 65 260 80 260 80 L275 210 L275 400 L65 400 L65 210 Z" fill={fill} stroke={shadow} strokeWidth="1.5" />
        <path d="M150 80 C155 90 165 96 170 96 C175 96 185 90 190 80 C185 72 175 68 170 68 C165 68 155 72 150 80 Z"              fill={shadow} stroke={shadow} strokeWidth="1" />
        <path d="M80 80 C80 80 100 65 140 60 L150 80 C155 90 165 96 170 96 C175 96 185 90 190 80 L200 60 C240 65 260 80 260 80 L275 210 L275 400 L65 400 L65 210 Z" fill="url(#tshirt-sheen)" stroke="none" />
...
/* ─────────────────────────────────────────────
   Print Zone (shared for front + back)
───────────────────────────────────────────── */
function PrintZone({
  artworkSrc, zoneStyle, isHovering,
  onHoverEnter, onHoverLeave, onPointerDown,
  onRemove, locationLabel,
}: {
...
export default function CustomizePage() {
  ...
  const [activeTab, setActiveTab]             = useState<"artwork" | "placement" | "color">("artwork");
  ...
  const applyPosFromPad = useCallback((clientX: number, clientY: number) => {
    ...
    setPadX(x); setPadY(y); setPosX(x); setPosY(y);
  }, []);

  useEffect(() => {
    if (!dragging) return;
    const onMove = (e: PointerEvent) => {
      if (dragging === "pad") { applyPosFromPad(e.clientX, e.clientY); return; }
      const el = containerRef.current;
      if (!el) return;
      const rect = el.getBoundingClientRect();
      const x = Math.max(0, Math.min(100, ((e.clientX - rect.left)  / rect.width)  * 100));
      const y = Math.max(0, Math.min(100, ((e.clientY - rect.top)   / rect.height) * 100));
      const nx = Math.max(0, Math.min(100, x - dragOffset.current.ox));
      const ny = Math.max(0, Math.min(100, y - dragOffset.current.oy));
      setPosX(nx); setPosY(ny); setPadX(nx); setPadY(ny);
    };
  ...
  const selectLocation = (loc: Location) => {
    ...
    setPosX(cx); setPosY(cy); setPadX(cx); setPadY(cy);
  };
  ...
  const handleZonePointerDown = (e: React.PointerEvent) => {
    ...
    const rect = el.getBoundingClientRect();
    dragOffset.current = {
      ox: ((e.clientX - rect.left)  / rect.width)  * 100 - posX,
      oy: ((e.clientY - rect.top)   / rect.height) * 100 - posY,
    };
    setDragging("zone");
  };

  const handleRemoveArtwork = (e: React.MouseEvent) => {
    e.stopPropagation();
    setArtworkSrc(null); setArtworkName(""); setArtworkFileSize(""); setIsHoveringZone(false);
  };
    ...
    <div className={styles.page}>
            ...
            <div className={`${styles.tshirtFlipInner} ${isFlipped ? styles.flipped : ""}`}>
              ...
              <div className={styles.tshirtFace}>
                <div className={styles.tshirtSvgContainer} ref={containerRef}>
                  ...
                  {view === "front" && (
                    <PrintZone
                      artworkSrc={artworkSrc} zoneStyle={zoneStyle}
                      isHovering={isHoveringZone}
                      onHoverEnter={() => setIsHoveringZone(true)}
                      onHoverLeave={() => setIsHoveringZone(false)}
      ...
      <section className={styles.controlsPanel}>
        ...
        <div className={styles.tabBar}>
          {(["artwork", "placement", "color"] as const).map((tab) => (
            <button
              key={tab}
          ...
          <div className={styles.tabContent}>
            {artworkSrc ? (
              ...
              >
                ...
                <div className={styles.uploadSubtitle}>or tap to browse • PNG, JPG up to 50 MB</div>
              </div>
            )}
            <input
            ...
            {/* Scale slider — shown when artwork is loaded */}
            {artworkSrc && (
              <div className={styles.sliderBlock}>
                <div className={styles.sliderBlockLabel}>
                  <span>Scale</span>
                  <span className={styles.sliderValue}>{scale}%</span>
                </div>
                <div className={styles.sliderRow}>
                  <span className={styles.sliderMark}>−</span>
                  <input
            ...
            {/* Position pad — shown when artwork is loaded */}
            {artworkSrc && (
              <div className={styles.padBlock}>
                ...
                <div
                  ...
                  onPointerDown={(e) => { applyPosFromPad(e.clientX, e.clientY); setDragging("pad"); }}
                >
                  <div className={styles.positionDot} style={{ left: `${padX}%`, top: `${padY}%` }} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className={styles.tabContent}>
            <div className={styles.colorGrid}>
              {GARMENT_COLORS.map((c) => (
                <button
                  key={c.name}
                  className={`${styles.colorOption} ${garmentColor.name === c.name ? styles.colorOptionActive : ""}`}
                  onClick={() => setGarmentColor(c)}
                >
                  <span
                    className={`${styles.colorSwatch} ${c.isWhite ? styles.colorSwatchWhite : ""}`}
                    style={{
                      backgroundColor: c.hex,
                      borderColor: garmentColor.name === c.name ? "var(--color-black)" : "transparent",
                    }}
                  >
                    {garmentColor.name === c.name && <span className={styles.checkMark}>✓</span>}
                  </span>
